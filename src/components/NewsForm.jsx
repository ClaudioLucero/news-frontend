...
const NewsForm = ({ initialData = {}, onSubmit, onClose }) => {
    ...
    return (
        <div className={`modal-overlay ${isDarkMode ? 'dark' : 'light'}`} onClick={handleOutsideClick}>
            <div className={`news-form-modal`}>
                <form className="news-form" onSubmit={handleSubmit} ref={formRef}>
                    ...
                    <button type="submit">{initialData.title ? 'Guardar Cambios' : 'Agregar Noticia'}</button>
                </form>
            </div>
        </div>
    );
